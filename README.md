...
- **Documentos** — biblioteca de documentos e manuais da empresa (Manual do Sistema, POPs operacionais, Política de Privacidade/LGPD, Brandbook, Código de Conduta, etc.): **31 documentos em 9 categorias** (Atendimento, Comercial, Cultura, Gestão, Jurídico, Marca, Operacional, RH, Segurança), exibidos em um **accordion por categoria** — cada categoria mostra a contagem de documentos e pode ser expandida/recolhida; o conjunto de categorias abertas fica salvo em `localStorage` e é restaurado ao voltar à página. A busca por título ou descrição expande automaticamente as categorias com resultado e oculta as demais. O botão "Abrir" navega para dentro do próprio sistema quando o documento é uma rota interna (ex: Matriz RACI) ou abre em nova aba para links externos. Admin gerencia (criar, editar, ativar/desativar) em Gerenciar → Configurações — sem precisar de código.
- **Matriz RACI** (`/raci`) — tabela interativa de responsabilidades por cargo: 31 processos organizados em 5 áreas (Técnica, Atendimento, Financeira, Comercial, Gestão) cruzados com 5 cargos (Diretor Geral, Gestão Técnica, Gestão Comercial, Atendimento/Financeiro, Técnico de Campo). Cabeçalho de colunas e coluna de processos ficam fixos (`sticky`) ao rolar nas duas direções; células coloridas por letra RACI (R = verde, A = azul, C = amarelo, I = cinza), com legenda no rodapé. Botão "Exportar PDF" usa `window.print()` com CSS de impressão em paisagem. Acessível como documento especial na categoria Gestão em `/documentos`, ou diretamente pela URL.
