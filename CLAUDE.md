...
- A ordem dos itens do menu em AppLayout.tsx é: Dashboard (admin only), Minhas Tarefas, Kanban, Calendário, Prospecção (admin+gestor_comercial), Scripts, Tickets, Estoque, Gerenciar (admin only), Documentos, Recursos, Ajuda. Manter essa ordem ao adicionar novos itens. "Notificações" não é item de menu — o sino ao lado do nome do usuário na sidebar já navega para /notificacoes, então incluí-lo no array `navigation` seria redundante.
...
- Accordion de documentos: separar estado da busca do localStorage evita que busca anterior contamine estado manual ao recarregar. Ao limpar a busca, restaurar exatamente o estado manual anterior.
- ICON_MAP de ícones lucide-react dinâmicos deve ser mantido completo — ícones não mapeados caem silenciosamente no fallback sem erro visível, só percebido quando a lista cresce.
- Item de menu redundante com elemento visual (sino) deve ser removido — dois caminhos para o mesmo destino poluem a navegação sem agregar valor.
