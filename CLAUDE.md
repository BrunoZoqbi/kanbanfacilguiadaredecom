...
- Módulos Documentos (/documentos) e Recursos (/recursos) são páginas de referência gerenciadas pelo Admin via painel interno — sem código. Ao adicionar novo item, o Admin usa a aba Gerenciar dentro de cada página. URLs marcadas como '#' são placeholders aguardando links reais do Google Drive ou storage externo.
