...
serve(async (req) => {
  ...
  try {
    ...
    // perform the email change itself.
    ...
    if (callerRole?.role !== "admin") {
      return new Response(
        JSON.stringify({ error: "Apenas administradores podem alterar o e-mail de outros usuários" }),
    ...
    // email_confirm: true — o admin definiu o e-mail por conta do usuário,
    // então já confirma automaticamente, para ele não ficar bloqueado
    // esperando confirmar um endereço que não escolheu.
